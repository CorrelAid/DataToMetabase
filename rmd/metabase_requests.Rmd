...
we load them from the environment, where they where added by the `.Renviron`
...
In order to do anything interesting we'll first need to get a session token for metabase.
...
Note the usage of `message_for_status` which we use purely to illustrate the
...
long to discuss here. Nevertheless we noticed three important things.
...
"query".
  ...
  - Query refers to the Query Builder, the graphical interface for asking questions.
